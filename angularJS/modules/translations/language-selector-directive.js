...
angular.module('app')
  .directive('languageSelector',function($localization,$rootScope,$timeout,$window){
    return {
      restrict: 'A',
      template:
        '<md-select ng-model="language" class="language-selector">' +
          '<md-option ng-value="lang" ng-repeat="lang in languages" class="language-selector-item"><img value="{{ lang.code }}" ng-src="{{ lang.icon }}" /> </md-option>' +
        '</md-select>',
      scope: true,
        ...
        if (scope.languages.length > 0) {
          ...
          $timeout(function() {
              ...
              $.each(items,function(i,item) {
                  $(item).click(function() {
                      $localization.setLanguage($(this).attr('value'));
                      $rootScope.$broadcast('$language:change');
