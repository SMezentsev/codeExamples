...
class Rafarma7Report extends Action
{
    ...
    public function run($equipid, $bdate, $edate, $nomenclature = null, $shift = null, $download = false, $type = '')
    {
        ...
        if ($download) {
            ...
            $report->save();
            \app\modules\itrack\models\AuditLog::Audit(\app\modules\itrack\models\AuditOperation::OP_REPORT_FILE, 'Заказ выгрузки отчета Сериализация', [
            ...
            \app\modules\itrack\models\AuditLog::Audit(\app\modules\itrack\models\AuditOperation::OP_REPORT, 'Просмотр отчета Сериализация', [
                ["field" => "Период с", "value" => $bdate],
                ["field" => "Период по", "value" => $edate],
                ["equipid" => "Идентификатор оборудования", "value" => $equipid],
                ["nomenclature" => "Номенклатура", "value" => $nomenclature],
